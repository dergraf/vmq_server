...
queue_crash_test(_) ->
    ...
    {ok, [1]} = vmq_reg:subscribe(false, false, <<"mock-user">>, SubscriberId,
                                  [{[<<"test">>, <<"topic">>], 1}]),
...
queue_lifo_test(_) ->
    ...
    {ok, false, QPid} = vmq_reg_leader:register_subscriber(SessionPid1, SubscriberId, QueueOpts),
    {ok, [1]} = vmq_reg:subscribe(false, false, <<"mock-user">>, SubscriberId, [{[<<"test">>, <<"lifo">>, <<"topic">>], 1}]),
    %% teardown session
    SessionPid1 ! go_down,
    timer:sleep(10),
...
queue_fifo_offline_drop_test(_) ->
    ...
    {ok, false, QPid} = vmq_reg_leader:register_subscriber(SessionPid1, SubscriberId, QueueOpts),
    {ok, [1]} = vmq_reg:subscribe(false, false, <<"mock-user">>, SubscriberId, [{[<<"test">>, <<"fifo">>, <<"topic">>], 1}]),
    %% teardown session
    SessionPid1 ! go_down,
    timer:sleep(10),
...
queue_offline_transition_test(_) ->
    ...
    SessionPid1 = spawn(fun() -> mock_session(Parent) end),
    {ok, false, QPid} = vmq_reg_leader:register_subscriber(SessionPid1, SubscriberId, QueueOpts),
    {ok, [1]} = vmq_reg:subscribe(false, false, <<"mock-user">>, SubscriberId, [{[<<"test">>, <<"transition">>], 1}]),
